...
async function ensureImportedAndUsed(ctx, packageName) {
    ...
    if (!imported && ctx.importPackage) {
        ...
        if (message.includes("error") || message.includes("failed") || message.includes("not found")) {
            throw new Error(asText(result) || "import package failed");
        }
    }
    if (ctx.usePackage) {
...
function Screen(ctx) {
    ...
    const tmuxPreviewState = useStateValue(ctx, "tmuxPreview", "点击上方 tab 查看窗口输出");
    const tmuxLinesState = useStateValue(ctx, "tmuxLines", "300");
    const busyState = useStateValue(ctx, "busy", false);
    const statusState = useStateValue(ctx, "status", "待操作");
    ...
    const callLinuxTool = async (toolName, params) => {
        ...
        throw new Error(lastError || `Failed to call ${toolName}`);
    };
    const runAction = async (title, action) => {
        ...
        try {
            await action();
            outputState.set("");
            statusState.set(`${title}完成`);
        }
        catch (error) {
            statusState.set(`${title}失败`);
    ...
    const captureTmuxWindow = async (windowName) => {
        return await callLinuxTool("linux_ssh_tmux_capture", Object.assign(Object.assign({}, getConnectionParams()), { tmux_session_name: tmuxSessionState.value.trim(), window_name: windowName, max_lines: parseOptionalPositiveInt(tmuxLinesState.value) || 300 }));
    };
    const refreshTmuxTabsAction = async () => {
        await runAction("同步 tmux 会话", async () => {
            var _a;
            const sessionName = tmuxSessionState.value.trim();
            if (!sessionName) {
                throw new Error("tmux 会话不能为空");
            }
            await saveCurrentConfigToEnv();
            const listResult = await callLinuxTool("linux_ssh_tmux_list_windows", Object.assign(Object.assign({}, getConnectionParams()), { tmux_session_name: sessionName }));
            ...
            tmuxPreviewState.set(previewText || "窗口暂无输出");
            return { listResult, captureResult, tabs };
        });
    };
    const createTmuxTabAction = async () => {
        await runAction("新建 tmux tab", async () => {
            const sessionName = tmuxSessionState.value.trim();
            if (!sessionName) {
                throw new Error("tmux 会话不能为空");
            }
            ...
            const createResult = await callLinuxTool("linux_ssh_exec", Object.assign(Object.assign({}, getConnectionParams()), { command: createScript }));
            selectedTmuxTabState.set(windowName);
            const listAfterCreateResult = await callLinuxTool("linux_ssh_tmux_list_windows", Object.assign(Object.assign({}, getConnectionParams()), { tmux_session_name: sessionName }));
    ...
    const deleteTmuxTabAction = async () => {
        await runAction("删除当前 Tab", async () => {
            var _a;
            const sessionName = tmuxSessionState.value.trim();
            ...
            const removeResult = await callLinuxTool("linux_ssh_exec", Object.assign(Object.assign({}, getConnectionParams()), { command: removeScript }));
            const listAfterDeleteResult = await callLinuxTool("linux_ssh_tmux_list_windows", Object.assign(Object.assign({}, getConnectionParams()), { tmux_session_name: sessionName }));
    ...
    const selectTmuxTabAction = async (windowName) => {
        ...
        selectedTmuxTabState.set(windowName);
        await runAction("加载 tmux 窗口", async () => {
            await saveCurrentConfigToEnv();
            const captureResult = await captureTmuxWindow(windowName);
            const previewText = getToolOutputText(captureResult).trim();
            tmuxPreviewState.set(previewText || "窗口暂无输出");
    ...
    const refreshSelectedTmuxWindowAction = async () => {
        const windowName = selectedTmuxTabState.value.trim();
        if (!windowName) {
            await runAction("刷新 tmux 预览", async () => {
                throw new Error("请先选择一个 tmux tab");
            });
            return;
        }
        await runAction("刷新 tmux 预览", async () => {
    ...
    const runTmuxCommandAction = async () => {
        await runAction("执行 tmux 命令", async () => {
            const sessionName = tmuxSessionState.value.trim();
            ...
            const runResult = await callLinuxTool("linux_ssh_exec", Object.assign(Object.assign({}, getConnectionParams()), { command: sendScript }));
            const captureResult = await captureTmuxWindow(windowName);
            const previewText = getToolOutputText(captureResult).trim();
            tmuxPreviewState.set(previewText || "窗口暂无输出");
            tmuxCommandState.set("");
    ...
    return ctx.UI.LazyColumn({
        ...
        ctx.UI.Surface({
            ...
            ctx.UI.Row({ padding: { horizontal: 12, vertical: 10 }, verticalAlignment: "center" }, [
                ...
                ctx.UI.Text({
                    text: "在执行了一次 linux ssh 工具之后，你可以直接给 AI 提供一些登录参数。这些参数会自动记录，无需手动在这个页面填写。",
                    style: "bodySmall",
        ...
        ...(() => {
                    ...
                    configExpandedState.value ? ctx.UI.Column({ spacing: 12 }, [
                        ...
                        ctx.UI.Row({ spacing: 8 }, [
                            ctx.UI.Surface({
                                ...
                                ctx.UI.Column({ padding: 12, spacing: 8 }, [
                                    ctx.UI.Text({ text: "端口", style: "bodyMedium", fontWeight: "medium" }),
                                    ctx.UI.Text({ text: "SSH 端口号", style: "bodySmall", color: "onSurfaceVariant" }),
                                    ctx.UI.Spacer({ height: 4 }),
                                    ctx.UI.Surface({
                                        fillMaxWidth: true,
                                        shape: { cornerRadius: 6 },
                            ...
                            ctx.UI.Surface({
                                ...
                                ctx.UI.Column({ padding: 12, spacing: 8 }, [
                                    ctx.UI.Text({ text: "用户名", style: "bodyMedium", fontWeight: "medium" }),
                                    ctx.UI.Text({ text: "SSH 登录用户名", style: "bodySmall", color: "onSurfaceVariant" }),
                                    ctx.UI.Spacer({ height: 4 }),
                                    ctx.UI.Surface({
                                        fillMaxWidth: true,
                                        shape: { cornerRadius: 6 },
                        ...
                        ctx.UI.Surface({
                            ...
                            ctx.UI.Column({ padding: 12, spacing: 8 }, [
                                ctx.UI.Text({ text: "密码（可选）", style: "bodyMedium", fontWeight: "medium" }),
                                ctx.UI.Text({ text: "留空则使用私钥认证", style: "bodySmall", color: "onSurfaceVariant" }),
                                ctx.UI.Spacer({ height: 4 }),
                                ctx.UI.Surface({
                                    fillMaxWidth: true,
                                    shape: { cornerRadius: 6 },
                        ...
                        ctx.UI.Surface({
                            ...
                            ctx.UI.Column({ padding: 12, spacing: 8 }, [
                                ctx.UI.Text({ text: "私钥路径（可选）", style: "bodyMedium", fontWeight: "medium" }),
                                ctx.UI.Text({ text: "SSH 私钥文件的完整路径", style: "bodySmall", color: "onSurfaceVariant" }),
                                ctx.UI.Spacer({ height: 4 }),
                                ctx.UI.Surface({
                                    fillMaxWidth: true,
                                    shape: { cornerRadius: 6 },
                        ...
                        ctx.UI.Row({ spacing: 8 }, [
                            ctx.UI.Surface({
                                ...
                                ctx.UI.Column({ padding: 12, spacing: 8 }, [
                                    ctx.UI.Text({ text: "tmux 会话", style: "bodyMedium", fontWeight: "medium" }),
                                    ctx.UI.Text({ text: "会话名称", style: "bodySmall", color: "onSurfaceVariant" }),
                                    ctx.UI.Spacer({ height: 4 }),
                                    ctx.UI.Surface({
                                        fillMaxWidth: true,
                                        shape: { cornerRadius: 6 },
                            ...
                            ctx.UI.Surface({
                                ...
                                ctx.UI.Column({ padding: 12, spacing: 8 }, [
                                    ctx.UI.Text({ text: "超时时间", style: "bodyMedium", fontWeight: "medium" }),
                                    ctx.UI.Text({ text: "毫秒", style: "bodySmall", color: "onSurfaceVariant" }),
                                    ctx.UI.Spacer({ height: 4 }),
                                    ctx.UI.Surface({
                                        fillMaxWidth: true,
                                        shape: { cornerRadius: 6 },
            ...
            const tmuxPreviewLinesRaw = (tmuxPreviewState.value || "点击上方 tab 查看输出").replace(/\r/g, "").split("\n");
            const tmuxPreviewLines = tmuxPreviewLinesRaw.length > 0 ? tmuxPreviewLinesRaw : ["点击上方 tab 查看输出"];
                    ...
                    tmuxPanelExpandedState.value ? ctx.UI.Column({ spacing: 12 }, [
                        ...
                        ctx.UI.Row({ spacing: 8 }, [
                            ctx.UI.Surface({
                                ...
                                ctx.UI.Column({ padding: 12, spacing: 8 }, [
                                    ctx.UI.Text({ text: "命令输入", style: "bodyMedium", fontWeight: "medium" }),
                                    ctx.UI.Text({ text: "将命令发送到选中的 tmux tab", style: "bodySmall", color: "onSurfaceVariant" }),
                                    ctx.UI.Spacer({ height: 4 }),
                                    ctx.UI.Surface({
                                        fillMaxWidth: true,
                                        shape: { cornerRadius: 6 },
                                        ...
                                        ctx.UI.TextField({
                                            ...
                                            placeholder: "例如: tail -f /var/log/syslog",
                                            minLines: 2,
                                            style: { fontSize: 14, fontWeight: "semiBold", color: "primary" }
                                        })
                                    ])
                            ...
                            ctx.UI.Surface({
                                ...
                                ctx.UI.Column({ padding: 12, spacing: 8 }, [
                                    ctx.UI.Text({ text: "预览行数", style: "bodyMedium", fontWeight: "medium" }),
                                    ctx.UI.Text({ text: "capture 行数", style: "bodySmall", color: "onSurfaceVariant" }),
                                    ctx.UI.Spacer({ height: 4 }),
                                    ctx.UI.Surface({
                                        fillMaxWidth: true,
                                        shape: { cornerRadius: 6 },
